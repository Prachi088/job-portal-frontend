...
// Must match the constant in NotificationsPage.jsx / ConnectPage.jsx
...
export default function ConnectionsPage() {
  ...
  // ── Load connections ───────────────────────────────────────────────────────
  // useCallback gives a stable reference so it can be used in both effects
  // and the event listener without causing infinite re-render loops.
  ...
  // NOTE: we do NOT gate on a specific pathname string here because the route
  // may be /connections, /connected, or whatever the app router defines.
  // location.pathname changing while this page is mounted is enough of a signal.
  useEffect(() => {
    loadConnections();
  }, [location.pathname, loadConnections]);

  // ── Listen for real-time accept events fired by NotificationsPage ─────────
  // This makes the connections list update immediately when the user accepts a
  // request in the Notifications page without requiring a full page reload.
  useEffect(() => {
    const handleConnectionUpdate = (e) => {
      const { type } = e.detail || {};
      if (type === "ACCEPTED") {
        // A new connection was just created — re-fetch the full list so the
        // newly connected person appears immediately.
        loadConnections();
      }
    };
      ...
      <div style={{
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "64px 20px", color: "var(--text-muted)" }}>
            <svg
              width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2"
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <span style={{
                        ...
                        background: conn.role === "RECRUITER"
                          ? "rgba(232,130,12,0.1)" : "rgba(79,70,229,0.1)",
                        color: conn.role === "RECRUITER" ? "#C26B06" : "#4F46E5",
                      }}>
