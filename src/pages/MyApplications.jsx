...
export default function MyApplications() {
  ...
  const [jobMap, setJobMap] = useState({})   // FIX: keyed map instead of array for O(1) lookup
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError('')
      try {
        // FIX: validate user ID before hitting the API
        if (!user?.id || String(user.id) === 'null' || String(user.id) === 'undefined') {
        ...
        // FIX: build a Map keyed by job.id for O(1) lookup in render instead of
        // calling Array.find() on every render cycle for each application row.
        const map = {}
  ...
  // FIX: use the map for O(1) lookup
  const getJob = (jobId) => jobMap[jobId] || null
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: 'clamp(16px, 4vw, 28px) clamp(14px, 4vw, 24px)' }}>
        ...
        {/* Error state */}
        {error && (
          <div style={{ background: 'var(--error-bg)', border: '1px solid var(--error-border)', color: 'var(--error)', padding: '12px 16px', borderRadius: 12, marginBottom: 20, fontSize: 13 }}>
            {error}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {applications.map(app => {
              ...
              // FIX: guard against missing job (job may have been deleted after application)
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(14px, 2vw, 16px)', fontWeight: 500, color: 'var(--text-primary)', marginBottom: 2 }}>
                      {/* FIX: show fallback text when job has been deleted */}
                      {job?.title || `Job #${app.jobId} (removed)`}
                    </h3>
